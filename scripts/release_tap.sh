...
echo ""
echo "  Next: commit the Cask to the Tap repo if you haven't already:"
echo "    cd \$(mktemp -d) && git clone git@github.com:${TAP_REPO}.git"
echo "    cd homebrew-sysmon && mkdir -p Casks"
echo "    cp dist/sysmon.rb Casks/sysmon.rb"
echo "    git add Casks/ && git commit -m 'Add sysmon v${VERSION}' && git push"
echo ""
echo "============================================================================="